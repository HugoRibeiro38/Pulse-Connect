...
const Chat = () => {
  ...
  return (
    <div className="chat-container w-full h-full flex flex-col overflow-hidden">
      <div className="user-info p-2 flex items-center">
        <Avatar>
      ...
      <div className="chat-window flex-1 overflow-y-auto">
        {messages.map((message, index) => (
          <div
            key={index}
            className={`message-box ${message.sender === 'user' ? 'flex justify-end' : 'flex justify-start'
              } my-2`}
          >
            <div
              className={`max-w-xs p-4 ${message.sender === 'user' ? 'bg-transparent border border-gray-500 text-gray-300' : 'bg-orange-500 text-white'
                } rounded-lg`}
            >
              {message.text}
            </div>
      ...
      <div className="message-input flex ">
        ...
        />
        <button onClick={sendMessage} className="ml-2 bg-orange-500 text-white p-2 rounded-md w-40">
          Send
        </button>
      </div>
    </div>
